...
# st.set_page_config removido para execução dentro do Control Center
...
def mascara_forno_eletrico_df(df: pd.DataFrame) -> pd.Series:
    texto = (
        df["DESCRIÇÃO DO PRODUTO"].map(normalizar_texto_busca)
        + " "
        + df["PRODUTO"].map(normalizar_texto_busca)
    )
    # Usa FORNO para pegar variações como FORNO ELETRICO, FORNO ELÉTRICO, FORNO 45L etc.
    return texto.str.contains("FORNO", na=False)
...
def aplicar_filtros(df: pd.DataFrame) -> pd.DataFrame:
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("🔥 Filtro FORNO ELÉTRICO")
    opcoes_forno = ["Geral", "Somente FORNO ELÉTRICO", "Sem FORNO ELÉTRICO"]
    if st.session_state.get("modo_pareto_forno") not in opcoes_forno:
        st.session_state["modo_pareto_forno"] = "Geral"
    modo_sidebar = st.sidebar.radio(
        "Aplicar no dashboard e no Pareto",
        opcoes_forno,
        index=opcoes_forno.index(st.session_state.get("modo_pareto_forno", "Geral")),
        key="modo_pareto_forno",
    )
    ...
    if not df_resultado.empty and modo_forno != "Geral":
        mascara_forno = mascara_forno_eletrico_df(df_resultado)
        if modo_forno == "Somente FORNO ELÉTRICO":
            df_resultado = df_resultado.loc[mascara_forno].copy()
...
def main():
    ...
    with aba3:
        ...
        mascara_forno_eletrico = mascara_forno_eletrico_df(df_pareto_base) if not df_pareto_base.empty else pd.Series(dtype=bool)

        qtd_forno = int(mascara_forno_eletrico.sum()) if not df_pareto_base.empty else 0
        valor_forno = df_pareto_base.loc[mascara_forno_eletrico, "VALOR"].sum() if not df_pareto_base.empty else 0
        ...
        st.markdown(
            ...
            <div class="info-card" style="margin-top: 2px;">
                ...
                Selecione a opção na barra lateral em <b>Filtro FORNO ELÉTRICO</b>.
            </div>
            """,
            unsafe_allow_html=True,
        )
