...
def _normaliza_texto(txt) -> str:
    s = "" if pd.isna(txt) else str(txt)
    s = s.upper().strip()
    repl = {
        "Á": "A", "À": "A", "Â": "A", "Ã": "A",
        "É": "E", "È": "E", "Ê": "E",
...
def _grupo_condicionado_por_mo(nome_grupo: str) -> bool:
    nome = _normaliza_texto(nome_grupo)
    return ("BANCADA" in nome) or ("EMBUTIR" in nome)
...
def _aplica_regra_mo_gargalo(df_agg: pd.DataFrame, col_nome: str, col_mod: str, col_util: str, mo_disponivel: float) -> pd.DataFrame:
    out = df_agg.copy()

    if col_util not in out.columns:
        out["_gargalo_liberado_"] = False
        out["_util_gargalo_"] = np.nan
        return out

    util_obj = out[col_util]
    if isinstance(util_obj, pd.DataFrame):
        util_series = util_obj.iloc[:, 0]
    else:
        util_series = util_obj
    ...
    if col_mod in out.columns:
        mod_obj = out[col_mod]
        if isinstance(mod_obj, pd.DataFrame):
            mod_series = mod_obj.iloc[:, 0]
        else:
            mod_series = mod_obj
        mod_series = pd.to_numeric(mod_series, errors="coerce").fillna(0.0)
    else:
        mod_series = pd.Series(0.0, index=out.index)

    nomes = out[col_nome].astype(str)
    condicionados = nomes.apply(_grupo_condicionado_por_mo)
    mo_insuficiente = mod_series > float(mo_disponivel)

    out["_gargalo_liberado_"] = (~condicionados) | mo_insuficiente
    out["_util_gargalo_"] = np.where(out["_gargalo_liberado_"], util_series, -1.0)
    out["_mod_base_gargalo_"] = mod_series
    return out
...
if "mo_disponivel_gargalo" not in st.session_state:
    st.session_state["mo_disponivel_gargalo"] = float(moi_total_fixo)

mo_disponivel_gargalo = float(st.session_state.get("mo_disponivel_gargalo", moi_total_fixo))
...
agg_exec["mod_pessoas"] = np.where(
    (MINUTOS_POR_PESSOA_DIA * float(dias_uteis)) > 0,
    (agg_exec["horas"] * 60.0) / (MINUTOS_POR_PESSOA_DIA * float(dias_uteis)),
    np.nan
)
agg_exec["mod_pessoas_proj"] = agg_exec["mod_pessoas"] * fator_previsao
agg_exec["score_ia"] = agg_exec["util_proj_pct"].fillna(0) + (agg_exec["qtd_modelos"].fillna(0) * 2.0)

agg_exec_gargalo_atual = _aplica_regra_mo_gargalo(agg_exec, "_GRUPO_EXEC_", "mod_pessoas", "util_pct", mo_disponivel_gargalo)
agg_exec_gargalo_prev = _aplica_regra_mo_gargalo(agg_exec, "_GRUPO_EXEC_", "mod_pessoas_proj", "util_proj_pct", mo_disponivel_gargalo)

gargalo_atual, gargalo_atual_util = _seleciona_gargalo_real(agg_exec_gargalo_atual, "_util_gargalo_", "_GRUPO_EXEC_")
gargalo_previsto, gargalo_previsto_util = _seleciona_gargalo_real(agg_exec_gargalo_prev, "_util_gargalo_", "_GRUPO_EXEC_")
...
rank_exec = _ranking_gargalos_reais(agg_exec_gargalo_atual, "_util_gargalo_")
rank_exec["meta"] = rank_exec.apply(
    lambda r: f"{_fmt_br(float(r['util_pct']),1)}% • {_fmt_br(float(r['horas']),2)} h • MO {_fmt_br(float(r['mod_pessoas']),2)}",
    axis=1
)
...
with aba0:
    ...
    st.number_input(
        "M.O disponível para linhas BANCADA / EMBUTIR",
        min_value=0.0,
        max_value=10000.0,
        step=1.0,
        key="mo_disponivel_gargalo",
        help="BANCADA e EMBUTIR só entram como gargalo quando a M.O necessária da linha for maior que este valor."
    )
    st.markdown(
        f"<div class='small-note'>Regra ativa: BANCADA e EMBUTIR só são tratadas como gargalo quando a utilização passar de 100% <b>e</b> a MOD necessária da linha for maior que a M.O disponível informada.</div>",
        unsafe_allow_html=True
    )
    ...
    with c2:
        ...
        if (rank_exec["_util_gargalo_"] > 100).any():
            _render_rank_bars(rank_exec, "_GRUPO_EXEC_", "_util_gargalo_", "meta", max_items=8)
        else:
            st.success("Sem gargalo no cenário atual. Nenhum recurso elegível está acima de 100% de utilização.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with c4:
        ...
        pred_chart_df = agg_exec_gargalo_prev.sort_values("_util_gargalo_", ascending=False).head(5).copy()
        pred_chart_df["Cor"] = pred_chart_df["_util_gargalo_"].apply(
        ...
        chart_pred_exec = alt.Chart(pred_chart_df).mark_bar(cornerRadiusEnd=8).encode(
            x=alt.X("_util_gargalo_:Q", title="Utilização projetada (%)"),
            y=alt.Y("_GRUPO_EXEC_:N", sort="-x", title=""),
            color=alt.Color("Cor:N", scale=None, legend=None),
            tooltip=[
                alt.Tooltip("_GRUPO_EXEC_:N", title="Descrição CR"),
                alt.Tooltip("util_proj_pct:Q", title="Utilização projetada", format=",.1f"),
                alt.Tooltip("mod_pessoas_proj:Q", title="MO projetada", format=",.2f"),
                alt.Tooltip("_gargalo_liberado_:N", title="Elegível gargalo"),
            ],
        ).properties(height=240)
...
with aba2:
    ...
    pessoas_disponiveis = float(st.session_state.get("mo_disponivel_gargalo", moi_total_fixo))
    st.markdown(
        f"<div class='small-note'>M.O disponível considerada no comparativo: <b>{_fmt_br(pessoas_disponiveis,0)}</b>. O ajuste é feito na aba <b>Resumo Executivo</b>.</div>",
        unsafe_allow_html=True
    )
    ...
    with c4:
        st.markdown(_card_html(
            ...
            "Valor ajustado na aba Resumo Executivo",
            "card-green"
        ), unsafe_allow_html=True)
    with c5:
        st.markdown(_card_html(
