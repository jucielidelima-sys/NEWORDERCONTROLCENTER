...
def get_producao_kpis(metas):
    ...
    fat_alias = {
        1: ["FAT.JAN", "FAT JAN", "FAT_JAN"],
        2: ["FAT.FEV", "FAT FEV", "FAT_FEV"],
        3: ["FAT.MAR", "FAT MAR", "FAT_MAR"],
        4: ["FAT.ABR", "FAT.ABRIL", "FAT ABR", "FAT ABRIL"],
        5: ["FAT.MAI", "FAT MAI", "FAT_MAI"],
        6: ["FAT.JUN", "FAT JUN", "FAT_JUN"],
        7: ["FAT.JUL", "FAT JUL", "FAT_JUL"],
        8: ["FAT.AGO", "FAT AGO", "FAT_AGO"],
        9: ["FAT.SET", "FAT SET", "FAT_SET"],
        10: ["FAT.OUT", "FAT OUT", "FAT_OUT"],
        11: ["FAT.NOV", "FAT NOV", "FAT_NOV"],
        12: ["FAT.DEZ", "FAT DEZ", "FAT_DEZ"],
    }
    for_alias = {
        1: ["FOR. JAN", "FOR.JAN", "FOR JAN"],
        2: ["FOR. FEV", "FOR.FEV", "FOR FEV"],
        3: ["FOR. MAR", "FOR.MAR", "FOR MAR"],
        4: ["FOR. ABR", "FOR.ABR", "FOR ABR", "FOR. ABRIL", "FOR.ABRIL"],
        5: ["FOR. MAI", "FOR.MAI", "FOR MAI"],
        6: ["FOR. JUN", "FOR.JUN", "FOR JUN"],
        7: ["FOR. JUL", "FOR.JUL", "FOR JUL"],
        8: ["FOR. AGO", "FOR.AGO", "FOR AGO"],
        9: ["FOR. SET", "FOR.SET", "FOR SET"],
        10: ["FOR. OUT", "FOR.OUT", "FOR OUT"],
        11: ["FOR. NOV", "FOR.NOV", "FOR NOV"],
        12: ["FOR. DEZ", "FOR.DEZ", "FOR DEZ"],
    }
    ...
    # Produzido: somente mês atual + FAB.RDO/FAB.BNU
    for sheet in sheets[:5]:
        ...
        prod_col = find_col(df, ["produç", "produzido", "qtd produzida", "qtde produzida", "produc"])
        data_col = find_col(df, ["data"])
        if not prod_col or prod_col not in df.columns or not data_col or data_col not in df.columns:
            continue

        # coluna classificadora
        filtro_col = None
        priority_cols = ["linha", "unidade", "filial", "fabrica", "fábrica", "origem", "tipo"]
        for key in priority_cols:
            c = find_col(df, [key])
            if c and c in df.columns:
                serie = df[c].astype(str).str.upper()
                if serie.str.contains(r"FAB\.RDO|FAB RDO|FAB\.BNU|FAB BNU", regex=True, na=False).any():
                    filtro_col = c
                    break
        if filtro_col is None:
            for c in df.columns:
                serie = df[c].astype(str).str.upper()
                if serie.str.contains(r"FAB\.RDO|FAB RDO|FAB\.BNU|FAB BNU", regex=True, na=False).any():
                    filtro_col = c
        ...
        dt = parse_date_series(df[data_col])
        mask_mes = (dt.dt.month == mes) & (dt.dt.year == ano)
        mask_fab = df[filtro_col].astype(str).str.upper().str.contains(r"FAB\.RDO|FAB RDO|FAB\.BNU|FAB BNU", regex=True, na=False)
        produzido_total += pd.to_numeric(df.loc[mask_mes & mask_fab, prod_col], errors="coerce").fillna(0).sum()

    # Faturado: usar somente coluna do mês atual; não cair em total
    ...
    # Forecast: usar somente coluna do mês atual
...
def get_pa_kpis(metas):
    ...
    for i, row in raw.iterrows():
        ...
        has_acao = vals.str.contains("ação|acao", case=False, na=False, regex=True).any()
        has_status = vals.str.contains("status", case=False, na=False).any()
        if has_acao or has_status:
            header_row = i
            if has_acao and has_status:
                break
    ...
    acao_col = find_col(df, ["ação", "acao", "plano de ação", "atividade", "descrição", "descricao"])
    status_col = find_col(df, ["status"])
    prazo_col = find_col(df, ["prazo", "data prazo", "vencimento", "deadline"])
    resp_col = find_col(df, ["responsável", "responsavel", "owner"])

    # total de ações = todas as linhas válidas com descrição de ação preenchida
    if acao_col and acao_col in df.columns:
        acao_series = df[acao_col].astype(str).str.strip()
        valid_mask = acao_series.ne("") & acao_series.ne("nan")
    else:
        # fallback: considera todas as linhas após limpeza
        valid_mask = pd.Series([True] * len(df), index=df.index)

    total_acoes = int(valid_mask.sum())

    # atrasadas = prazo vencido e não concluída/cancelada
    atrasadas = 0
    if prazo_col and prazo_col in df.columns:
        dt = parse_date_series(df[prazo_col])
        if status_col and status_col in df.columns:
            status_s = df[status_col].astype(str).str.strip().str.lower()
            closed = status_s.str.contains(r"executad|conclu|finaliz|cancel", regex=True, na=False)
        else:
            closed = pd.Series([False] * len(df), index=df.index)
        atrasadas = int(((dt < pd.Timestamp.today().normalize()) & (~closed) & valid_mask).sum())

    responsavel = "-"
    if resp_col and resp_col in df.columns:
        top = df.loc[valid_mask, resp_col].astype(str).str.strip()
    ...
    return {"abertas":total_acoes,"atrasadas":atrasadas,"responsavel":responsavel,"status":status}
...
def allowed_options(perfil):
    p = str(perfil).lower()
    if p == "diretoria":
        return ["Home","Produção","Plano de Ação Fábrica","Carga Máquina","RH • Pessoas","Scrap","MES • Gestão"]
    if p == "producao":
        return ["Home","Produção","Plano de Ação Fábrica","Carga Máquina","Scrap","MES • Gestão"]
    if p == "rh":
        return ["Home","RH • Pessoas","Scrap","MES • Gestão"]
    return ["Home"]
...
def home():
    ...
    with m5:
        module_card("♻️", "Qualidade", "Scrap", "Dashboard de sucata e retrabalho com refugo, retrabalho, Pareto, filtros por produto, depósito, entrada/saída e valor.")
...
if menu == "Home":
    ...
    run_legacy_app(LEGACY_DIR / "scrap" / "app.py")
