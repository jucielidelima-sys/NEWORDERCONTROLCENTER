...
with t2:
    ...
    with st.form("form_absenteismo", clear_on_submit=False):
        c1, c2 = st.columns(2)
        with c1:
        ...
        with c2:
            ...
            st.text_input("Cadastro", value=str(info["Cadastro"]), disabled=True)
            st.text_input("Cargo", value=str(info["Cargo"]), disabled=True)
            st.text_input("Setor", value=str(info["SETOR"]), disabled=True)
        obs_abs = st.text_area("Observação", key="obs_abs")
        enviar_abs = st.form_submit_button("Salvar absenteísmo", use_container_width=True)
        if enviar_abs:
            row = [
...
with t3:
    ...
    with st.form("form_turnover", clear_on_submit=False):
        c1, c2 = st.columns(2)
        with c1:
        ...
        with c2:
            ...
            st.text_input("Cadastro ", value=str(info_t["Cadastro"]), disabled=True)
            st.text_input("Cargo ", value=str(info_t["Cargo"]), disabled=True)
            st.text_input("Setor ", value=str(info_t["SETOR"]), disabled=True)
        obs_turn = st.text_area("Observação ", key="obs_turn")
        enviar_turn = st.form_submit_button("Salvar turnover", use_container_width=True)
        if enviar_turn:
            row = [
